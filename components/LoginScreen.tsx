...
export default function LoginScreen() {
  ...
  const indicatorStyle = useAnimatedStyle(() => {
    const w = switcherWidth > 0 ? (switcherWidth - 8) / 2 : 0;
    return { width: w, transform: [{ translateX: tabProgress.value * w }] };
  });
    ...
    <AuthShell aboveSheet={<Animated.View entering={FadeInDown.duration(600).easing(Easing.out(Easing.cubic))} style={styles.headerSection}>
      ...
      <Animated.View layout={Layout.springify().damping(18)} style={styles.form}>
        {!isLogin && <Animated.View entering={FadeInDown.duration(400).springify().damping(16)} exiting={FadeOutUp.duration(220)} layout={Layout.springify()} style={[styles.inputRow, { borderColor: theme.outline, backgroundColor: theme.surfaceVariant }]}>
          ...
          <TextInput placeholder="Full Name" placeholderTextColor={theme.onSurfaceVariant} value={name} onChangeText={setName} autoCapitalize="words" style={[styles.input, { color: theme.onSurface }]} />
        </Animated.View>}
        ...
        {isLogin && <Animated.View entering={FadeIn.duration(400).delay(320)} exiting={FadeOutUp.duration(180)} layout={Layout.springify()} style={styles.optionsRow}>
          <Pressable style={styles.rememberMe} onPress={() => setRememberMe((v) => !v)} hitSlop={8}>
            <View style={[styles.checkbox, { borderColor: theme.onSurfaceVariant }, rememberMe && { backgroundColor: theme.primary, borderColor: theme.primary }]}>{rememberMe && <Animated.View entering={ZoomIn.springify().damping(12)} exiting={ZoomOut.duration(120)}><Ionicons name="checkmark" size={12} color={theme.buttonText} /></Animated.View>}</View>
      ...
      <Animated.View layout={Layout.springify().damping(18)} style={styles.bottomSection}>
        <Animated.View entering={FadeInUp.duration(500).delay(340)} style={[styles.buttonWrapper, { shadowColor: theme.primary }, glowStyle]}>
          <Pressable onPress={handleSubmit} disabled={isSubmitting} onPressIn={() => (ctaScale.value = withSpring(0.96, SPRING))} onPressOut={() => (ctaScale.value = withSpring(1, SPRING))}>
            <Animated.View style={[styles.button, { backgroundColor: theme.primary, opacity: isSubmitting ? 0.65 : 1 }, ctaAnimStyle]}>
              <Text style={[styles.buttonText, { color: theme.buttonText }]}>{isSubmitting ? <ActivityIndicator color={theme.onSurface} /> : isLogin ? "Login" : "Create Account"}</Text>
            </Animated.View>
          </Pressable>
        </Animated.View>
        ...
        <View style={styles.socialRow}>
          <Animated.View entering={FadeInUp.duration(450).delay(480)} style={{ flex: 1 }}>
            <Pressable onPress={handleGoogleSignIn} disabled={isSubmitting} style={[styles.socialButton, { borderColor: theme.outline, backgroundColor: theme.surfaceVariant }]}>
              <Ionicons name="logo-google" size={18} color="#EA4335" /><Text style={[styles.socialButtonText, { color: theme.onSurface }]}>Google</Text>
            </Pressable>
          </Animated.View>
          <Animated.View entering={FadeInUp.duration(450).delay(540)} style={{ flex: 1 }}>
            <Pressable style={[styles.socialButton, { borderColor: theme.outline, backgroundColor: theme.surfaceVariant }]}>
              <Ionicons name="logo-apple" size={20} color={theme.onSurface} /><Text style={[styles.socialButtonText, { color: theme.onSurface }]}>Apple</Text>
            </Pressable>
          </Animated.View>
        </View>
      </Animated.View>
