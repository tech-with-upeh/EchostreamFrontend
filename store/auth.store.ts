...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  initialize: async () => {
    ...
    try {
      ...
      set({ isAuthenticated: false, isInitialized: true, error: error instanceof Error ? error.message : "Unable to restore session." });
    } finally {
      set({ isLoading: false });
    }
  },
  ...
  register: async (firstName, lastName, email, password) => {
    set({ isLoading: true, error: null });
    try { return await apiRegister(firstName, lastName, email, password); }
    catch (error) { set({ error: error instanceof Error ? error.message : "Unable to create your account." }); throw error; }
    finally { set({ isLoading: false }); }
  },
  ...
  verifyEmail: async (email, code) => {
    ...
    try {
      ...
      set({ isAuthenticated: true });
      return result;
    } catch (error) { set({ error: error instanceof Error ? error.message : "Unable to verify your email." }); throw error; }
    finally { set({ isLoading: false }); }
  },

  resendVerification: async (email) => {
    set({ isLoading: true, error: null });
    try { return await apiResendVerification(email); }
    catch (error) { set({ error: error instanceof Error ? error.message : "Unable to resend the verification code." }); throw error; }
    finally { set({ isLoading: false }); }
  },

  forgotPassword: async (email) => {
    set({ isLoading: true, error: null });
    try { return await ForgotPassword(email); }
    catch (error) { set({ error: error instanceof Error ? error.message : "Unable to send the reset code." }); throw error; }
    finally { set({ isLoading: false }); }
  },
  ...
  resetPassword: async (token, email, password) => {
    ...
    try {
      ...
      if (!result.access_token || !result.refresh_token) throw new Error("The server returned an invalid login response.");
      await persistAuthTokens(result);
      set({ isAuthenticated: true });
      return result;
    } catch (error) { set({ error: error instanceof Error ? error.message : "Unable to reset your password." }); throw error; }
    finally { set({ isLoading: false }); }
  },
  ...
  logout: async () => {
    set({ isLoading: true, error: null });
    try { await apiLogout(); }
    catch (error) { set({ error: error instanceof Error ? error.message : "Unable to log out." }); }
    finally {
      clearAuthTokens();
