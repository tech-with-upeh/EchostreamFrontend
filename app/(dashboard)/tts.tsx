...
const ALLOWED_USER_LABELS: Record<(typeof ALLOWED_USER_KEYS)[number], string> =
  {
    allUsers: "All Users",
    followers: "Followers",
...
export default function TtsScreen() {
  ...
  const filteredAlerts = alerts.filter((a) =>
    a.label.toLowerCase().includes(alertQuery.trim().toLowerCase()),
  );
  ...
  const filteredGifts = useMemo(
    () =>
      TIKTOK_GIFTS.filter((g) =>
        `${g.name} ${g.category}`
          .toLowerCase()
          .includes(giftQuery.trim().toLowerCase()),
      ),
    [giftQuery],
  );
  ...
  const addGiftAlert = () => {
    ...
    setAlerts((prev) =>
      prev.some((a) => a.giftId === selectedGift.id)
        ? prev
        : [
            ...prev,
            {
              ...
              label: selectedGift.isAnyGift
                ? "Any Gift"
                : `${selectedGift.name}${selectedGift.coins ? ` (${selectedGift.coins} Coins)` : ""}`,
              enabled: true,
            },
          ],
    );
  ...
  const premiumLabel = (text: string) => (
    <View style={styles.paidLabelRow}>
      <Text style={{ color: theme.onSurface }}>{text}</Text>
      <MaterialCommunityIcons name="crown" size={12} color="#FFB961" />
    </View>
  );
          ...
          <>
            ...
            >
              ...
              <SliderBlock
                label="Pitch"
                value={pitch}
                display={`${pitch.toFixed(2)}x`}
                min={0.5}
                max={2}
                setValue={setPitch}
                left="musical-note-outline"
                right="musical-notes-outline"
                theme={theme}
              />
            ...
            >
              ...
              />
              <View
                style={[styles.rowDivider, { backgroundColor: theme.outline }]}
              />
            ...
            >
              {ALLOWED_USER_KEYS.map((key, index) => (
                <React.Fragment key={key}>
                  <ToggleRow
                    label={ALLOWED_USER_LABELS[key]}
                    value={allowedUsers[key]}
                    onValueChange={() => toggleAllowedUser(key)}
                    disabled={key !== "allUsers" && allowedUsers.allUsers}
                    bold={key === "allUsers"}
                    isPremium={key !== "allUsers"}
                  />
                  {index < ALLOWED_USER_KEYS.length - 1 && (
                    <View
                      style={[
                        styles.rowDivider,
            ...
            >
              {filteredAlerts.map((alert, index) => (
                <React.Fragment key={alert.id}>
                  <ToggleRow
                    ...
                    onPress={() =>
                      router.push(`/sound-alert/${alert.id}` as any)
                    }
                    showChevron
                  />
              ...
              {!giftPickerOpen ? (
                ...
                >
                  ...
                  />
                  <Text
                    style={[styles.addAlertText, { color: theme.primaryDim }]}
                  >
                    Add new Sound Alert
                  </Text>
                </Pressable>
                ...
                <>
                  <View style={styles.pickerHeader}>
                    ...
                    <Pressable onPress={closeGiftPicker}>
                      <Ionicons
                        name="close"
                        size={20}
                        color={theme.onSurfaceVariant}
                    ...
                    <React.Fragment key={gift.id}>
                      ...
                      >
                        ...
                        <View style={{ flex: 1 }}>
                          ...
                          >
                            {gift.coins ? `${gift.coins} Coins` : gift.category}
                          </Text>
                        </View>
                  ...
                  {selectedGift && (
                    ...
                    >
                      <Text
                        style={[
                          styles.confirmGiftText,
                          { color: theme.buttonText },
                        ]}
                      >
                        Add {selectedGift.name}
                      </Text>
                    </Pressable>
                  )}
                </>
...
const styles = StyleSheet.create({
  ...
  headerIcon: { position: "absolute", left: 20 },
  headerTitle: { fontSize: 18, fontWeight: "700" },
  scrollContent: { paddingHorizontal: 20, paddingBottom: 140 },
  tabsWrapper: { marginBottom: 22 },
  sectionLabel: {
    fontSize: 12,
    fontWeight: "700",
  ...
  card: { borderRadius: 18, borderWidth: 1, padding: 16, marginBottom: 22 },
  voiceRow: {
    flexDirection: "row",
    alignItems: "center",
  ...
  pickerSub: { fontSize: 11, lineHeight: 16, marginTop: 3 },
  giftRow: {
    minHeight: 58,
    flexDirection: "row",
    alignItems: "center",
  ...
  giftEmoji: { fontSize: 25, width: 34, textAlign: "center" },
  giftName: { fontSize: 13, fontWeight: "700" },
  giftMeta: { fontSize: 11, marginTop: 2 },
  confirmGift: {
    borderRadius: 12,
    alignItems: "center",
    paddingVertical: 12,
  ...
  confirmGiftText: { fontSize: 13, fontWeight: "800" },
  paidLabelRow: { flexDirection: "row", alignItems: "center", gap: 5 },
});
