...
import { getCurrentUser, getLiveStatus } from "@/lib/api";
import { LiveStatus, UserProfile } from "@/lib/schema";
import {
  FontAwesome5,
  Ionicons,
    ...
    getLiveStatus()
      ...
      .finally(() => {
        if (Livestatus?.username != "") {
          setTiktokUsername(Livestatus?.username || "");
          if (Livestatus?.status == "started") {
            setIsActive(true);
          }
        }
...
function FeatureCard({
  ...
  return (
    ...
    >
      ...
      >
        <Text style={[styles.featureButtonText, { color: theme.buttonText }]}>
          {actionLabel}
        </Text>
      </Pressable>
    </View>
...
function VoiceCard({
  ...
  return (
    ...
    >
      ...
      <Text style={[styles.voiceStyle, { color: theme.onSurfaceVariant }]}>
        {voice.style}
      </Text>
...
function WaveBar({
  ...
  const barHeight = useSharedValue(BASE_HEIGHT);
  useEffect(() => {
    if (active) {
      ...
      barHeight.value = withRepeat(
        withSequence(
          ...
          withTiming(
            BASE_HEIGHT + Math.random() * (MAX_HEIGHT - BASE_HEIGHT) * 0.5,
      ...
      barHeight.value = withTiming(BASE_HEIGHT, { duration: 250 });
    }
  }, [active]);
...
const styles = StyleSheet.create({
  ...
  voiceStyle: { fontSize: 11, marginTop: 3 },
  voiceFooter: {
    flexDirection: "row",
    alignItems: "center",
    gap: 8,
