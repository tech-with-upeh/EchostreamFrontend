  ...
  return (
    <AuthShell showBack onBack={() => router.back()}>
      <View style={styles.centered}>
        ...
        <Animated.View entering={FadeInUp.duration(500).delay(360)} style={[styles.buttonWrapper, { shadowColor: theme.primary }, glowStyle]}>
          <Pressable onPress={handleSendCode} disabled={!isValid || isLoading} onPressIn={() => (ctaScale.value = withSpring(0.96, SPRING))} onPressOut={() => (ctaScale.value = withSpring(1, SPRING))}>
            <Animated.View style={[styles.button, { backgroundColor: theme.primary, opacity: isValid ? 1 : 0.5 }, ctaAnimStyle]}>
              {isLoading ? <ActivityIndicator color="#fff" animating={isLoading} /> : <Text style={[styles.buttonText, { color: theme.buttonText }]}>Send Reset Code</Text>}
            </Animated.View>
          </Pressable>
        </Animated.View>
        <Animated.View entering={FadeInDown.duration(400).delay(420)}>
          <Pressable onPress={() => router.back()}><Text style={[styles.backToLogin, { color: theme.primaryDim }]}>Back to Login</Text></Pressable>
        </Animated.View>
      </View>
    </AuthShell>
  );
...
const styles = StyleSheet.create({
  ...
  inputRow: { flexDirection: "row", alignItems: "center", gap: 10, width: "100%", paddingHorizontal: 14, paddingVertical: 12, borderWidth: 1, borderRadius: 12, marginBottom: 20 },
  input: { flex: 1, fontSize: 14, padding: 0 },
  buttonWrapper: { width: "100%", borderRadius: 9999, shadowOffset: { width: 0, height: 10 }, shadowRadius: 20, elevation: 10, marginBottom: 20 },
  ...
  backToLogin: { fontSize: 13, fontWeight: "600", marginBottom: 8 },
});
